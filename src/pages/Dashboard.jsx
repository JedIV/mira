...
const eventTypeConfig = {
  deploy: { color: 'bg-blue-500', icon: 'D' },
  alert: { color: 'bg-amber-500', icon: '!' },
  action: { color: 'bg-emerald-500', icon: 'A' },
  approval: { color: 'bg-primary-500', icon: 'OK' },
  scan: { color: 'bg-primary-500', icon: 'S' },
  test: { color: 'bg-emerald-500', icon: 'T' },
  health: { color: 'bg-emerald-500', icon: 'H' },
  metric: { color: 'bg-blue-500', icon: 'M' },
}
...
function TopAgentsTable() {
  return (
    <Card padding={false}>
      <div className="px-6 pt-5 pb-3 flex items-center justify-between rounded-t-lg" style={{ backgroundColor: '#F5F5F3' }}>
        <div>
          <h3 className="font-bold text-primary-700">Most Active Agents</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-y border-slate-400/90">
          ...
          <tbody className="divide-y divide-slate-400/90">
            {topAgents.map((agent, i) => {
              ...
              return (
                <tr key={agent.id} className="hover:bg-slate-100/60 transition-colors">
                  ...
                  <td className="px-3 py-2.5">
                    <Link to={`/agents/${agent.id}`}>
                      <ChevronRightIcon className="w-4 h-4 text-slate-400 hover:text-slate-600 transition-colors" />
                    </Link>
                  </td>
                </tr>
              )
...
function LiveTicker() {
  ...
  return (
    <div className="flex items-center gap-3 h-6 overflow-hidden">
      ...
      <span className="text-xs text-slate-500 flex-shrink-0 ml-auto">{visibleEvent.time}</span>
    </div>
  )
}
...
function PlatformTile({ source }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 px-3 py-2.5 bg-transparent border border-slate-400/90 hover:bg-slate-100/60 transition-colors group"
    >
      ...
      <ChevronRightIcon className="w-3.5 h-3.5 text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
    </Link>
  )
}
...
function StatusBoard() {
  ...
  return (
    <div className="card p-0 overflow-hidden">
      ...
      <div className="grid grid-cols-5 divide-x divide-slate-400/90 border-b border-slate-400/90">
        <Link to="/inventory" className="px-4 py-3 text-center hover:bg-slate-100/60 transition-colors">
          <p className="text-2xl font-bold text-slate-900">{totalAgents.toLocaleString()}</p>
          <p className="text-[10px] uppercase tracking-wider text-slate-500 font-medium">Total Agents</p>
        </Link>
        <Link to="/performance/operational" className="px-4 py-3 text-center hover:bg-slate-100/60 transition-colors">
          ...
          <p className="text-[10px] uppercase tracking-wider text-slate-500 font-medium">Op Status</p>
        </Link>
        ...
        <Link to="/governance" className="px-4 py-3 text-center hover:bg-slate-100/60 transition-colors">
          ...
          <p className="text-[10px] uppercase tracking-wider text-slate-500 font-medium">Pending Approvals</p>
        </Link>
      </div>
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          ...
          <Link
            to="/platforms/add"
            className="flex items-center justify-center gap-2 px-3 py-2.5 border-2 border-dashed border-slate-400/90 hover:border-primary-400 hover:bg-primary-50/30 transition-colors group"
          >
      ...
      {/* Live event ticker */}
      <div className="px-4 py-3 bg-slate-100/60 border-t border-slate-400/90">
        <LiveTicker />
      </div>
    </div>
  )
          ...
          {dimensions.map((dim) => (
            <Link key={dim.name} to={dim.href}>
              <Card hover className="h-full">
                ...
                <div className="flex items-center justify-between pt-3 border-t border-slate-400/90">
                  <div>
